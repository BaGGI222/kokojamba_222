...
class Ball:
    ...
    def touch_platform(self, ball_coords):
        ...
        if platform_coords[0] <= ball_coords[0] and ball_coords[2] <= platform_coords[2] and platform_coords[1] <= ball_coords[3] <= platform_coords[3]:
            return True
        return False
...
class Platform:
    def __init__(self, canvas, color):
        self.canvas = canvas
        self.rect = canvas.create_rectangle(230, 350, 330, 360, fill=color)
        self.x = 0
